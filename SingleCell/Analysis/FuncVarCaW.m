...
for i = 1:10
    ...
    for Number = 1:3
        Help(Number) = CaWVelo(j(Number));
    end
    
    CaWVelo2(i) = std(100./(Help'));
end
kmeansdata(:, 1, 1) = CaWVelo2;
...
kmeansdata(:, 4, 4) = std(100./(CaWVelo([3,5])'));
...
kmeansdata(:, 1, 3) = std(100./(CaWVelo([1:6,8:end])'));
...
kmeansdata(:, 2, 6) = std(100./(CaWVelo([1,2,3,5,8,9,10])'));
...
axis([1 6 0 .7]); %set(gca, 'YScale', 'log')
...
errorbar(5,mean(plotkmeansdata),std(plotkmeansdata))
plot(5,mean(plotkmeansdata),'x')
axis([1 6 0 .2]); %set(gca, 'YScale', 'log')
anovadata(3,:) = plotkmeansdata(:);
