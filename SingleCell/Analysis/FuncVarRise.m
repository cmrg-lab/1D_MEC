...
for i = 1:10
    ...
    for Number = 1:3
        Help(Number) = max(TimeStartDAD2(j(Number),:));
    end
    
    RiseTimeDAD(i) = std(Help);
end
kmeansdata(:, 1, 1) = RiseTimeDAD;
...
axis([1 6 0 120]); %set(gca, 'YScale', 'log')
...
errorbar(4,mean(plotkmeansdata([1,3:end])),std(plotkmeansdata([1,3:end])))
plot(4,mean(plotkmeansdata([1,3:end])),'x')
...
errorbar(1,mean(plotkmeansdata([1:7,9:end])),std(plotkmeansdata([1:7,9:end])))
plot(1,mean(plotkmeansdata([1:7,9:end])),'x')
anovadata(1,:) = plotkmeansdata(:);
