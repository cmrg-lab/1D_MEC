  ...
  void operator()( const state_type &x , state_type &dxdt , double t)
{
    for (int jCount = 0; jCount<cells; jCount++){
      for (int i = 0; i < segments; i++){
        ...
        //Sinus
        GaussCurve = -(2.0*pi/(400.0))*cos(2.0*pi*(t+300.0)/(400.0))/(10.0*exp(1.0/(216.0e+6) *pow((3600.0*10.0),2.0) - 1.0/(18.0e+3)*(3600.0*10.0)))*103.0;
        ...
        // internal shortening
        if (i != 0 && i != (segments-1)){
          dSL = -(dSL1[i-1] + dSL1[i+1] - 2.0*dSL1[i]) + GaussCurve;
        } else if (i == 0){
          dSL = -(dSL1[i+1] - dSL1[i]) + GaussCurve;
      ...
      /*if (jCount>0){
        // SL
        dxdt[45+ StateNum*(segments-1)+ StateNum*(segments*(jCount-1))] = dxdt[45+ StateNum*(segments-1)+ StateNum*(segments*(jCount-1))]
                                                              - 1.0/10.0*(dxdt[45+ StateNum*0+ StateNum*(segments*jCount)]
                                                              - dxdt[45+ StateNum*(segments-1)+ StateNum*(segments*(jCount-1))]);
        dxdt[45+ StateNum*0+ StateNum*(segments*jCount)] = dxdt[45+ StateNum*0+ StateNum*(segments*jCount)]
                                               + 1.0/10.0*(dxdt[45+ StateNum*0+ StateNum*(segments*jCount)]
                                               - dxdt[45+ StateNum*(segments-1)+ StateNum*(segments*(jCount-1))]);
...
int main(int argc, char **argv)
{
  ...
  for (int iCount = 0; iCount < cells; iCount++){
      std::string str = "SarcRef" + std::to_string(iCount+10) + ".txt";
      ifstream file(str);
      if(file.is_open())
      {
        for(int jCount = 0; jCount < segments; jCount++){
            file >> T2.SLsetAll[jCount + iCount*jCount];
        }
      }
      std::string strGap = "G_gap" + std::to_string(iCount+10) + ".txt";
      ifstream fileGap(strGap);
      if(fileGap.is_open())
      {
        for(int jCount = 0; jCount < segments; jCount++){
            fileGap >> T2.GapAll[jCount + iCount*jCount];
        }
      }
  }
  ...
  for (int i = 0; i < cells; i++){
    std::string str = "Shannon" + std::to_string(i+10) + ".txt";
    ifstream file(str);
    if(file.is_open())
    {
      for(int jCount = 0; jCount < segments; ++jCount)
      {
        for(int iCount = 0; iCount < StateNum; ++iCount)
        {
            ...
            if (iCount == 45){
              x[k-1] = T2.SLsetAll[jCount+iCount*i];
            }
        }
      }
    }
  ...
  for (int i = 1; i < 100; i++){
    cout << i<< endl;
    integrate( ODEShannon , x , 0.0 , 400.0 , 0.1);
  }
  for (int i = 1; i < 2; i++){
    cout << i<< endl;
    integrate( ODEShannon , x , 0.0 , 2200.0 , 0.1, write_Shannon);
  }
}
