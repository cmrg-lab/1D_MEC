...
struct OpT
{
    ...
    std::array<double, cells*segments> G_fibAll;
};

//output
double kPrint = 1800.0;
...
int main(int argc, char **argv)
{
  ...
  for (int iCount = 0; iCount < cells; iCount++){
      std::string str = "SarcRest" + std::to_string(iCount+1) + ".txt";
      ifstream file(str);
      if(file.is_open())
      {
        for(int jCount = 0; jCount < segments; jCount++){
            file >> T2.SLsetAll[kSarcRest];
            kSarcRest++;
        }
      }
      std::string strG_fib = "G_fib" + std::to_string(iCount+1) + ".txt";
      ifstream fileG_fib(strG_fib);
      if(fileG_fib.is_open())
      {
        for(int jCount = 0; jCount < segments; jCount++){
            fileG_fib >> T2.G_fibAll[kG_fib];
            kG_fib++;
        }
      }
  }
  ...
  int kInit = 0;
  int kSLset = 0;
    for (int iCount = 0; iCount < cells; i++){
